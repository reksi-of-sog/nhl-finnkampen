...
async function main() {
  ...
  // --- REVERTING TO ORIGINAL TWEET CONTENT (WITHOUT DEBUG ID) ---
  ...
  // --- END REVERTING TO ORIGINAL TWEET CONTENT ---
  ...
  if (process.env.TWITTER_ENABLE === '1') {
    ...
    try// filepath: c:\Users\Hercules Admin\NHL\scripts\post-nightly.ts
import 'dotenv/config'; // To load environment variables from .env
import { pool } from '../src/lib/db.js';
import { seasonFromDate } from '../src/lib/util.js';
import { TwitterApi } from 'twitter-api-v2';
...
async function main() {
  ...
  tweet += `#politiikka #urheilu #stockholm #ranska #joku #muu`;
  // --- END REVERTING TO ORIGINAL TWEET CONTENT ---
